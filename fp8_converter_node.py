...
class FP8ConverterNode:
    ...
    def convert_clip_to_fp8(self, clip):
        # CLIPオブジェクトのパラメータとバッファをFP8形式に変換
        for name, param in tqdm(clip.named_parameters(), desc="CLIPパラメータをFP8に変換中"):
            if param.dtype in [torch.float32, torch.float16]:
                with torch.no_grad():
                    param.copy_(param.to(dtype=torch.float8_e4m3fn))

        for name, buffer in tqdm(clip.named_buffers(), desc="CLIPバッファをFP8に変換中"):
